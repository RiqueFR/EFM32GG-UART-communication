...
int PWM_Init(TIMER_TypeDef *timer, int location, unsigned params)
{
    ...
    {
        timer->ROUTE |= (1 << 0);
        timer->CC[0].CTRL = TIMER_CC_CTRL_ICEVCTRL_RISING | TIMER_CC_CTRL_ICEDGE_RISING | TIMER_CC_CTRL_CMOA_CLEAR | TIMER_CC_CTRL_COIST | TIMER_CC_CTRL_MODE_PWM;
        timer->CC[0].CCVB = 0xFFFF;
    }
    if (params & PWM_PARAMS_ENABLECHANNEL1)
    {
        timer->ROUTE |= (1 << 1);
        timer->CC[1].CTRL = TIMER_CC_CTRL_ICEVCTRL_RISING | TIMER_CC_CTRL_ICEDGE_RISING | TIMER_CC_CTRL_CMOA_CLEAR | TIMER_CC_CTRL_COIST | TIMER_CC_CTRL_MODE_PWM;
        timer->CC[1].CCVB = 0xFFFF;
    }
    if (params & PWM_PARAMS_ENABLECHANNEL2)
    {
        timer->ROUTE |= (1 << 2);
        timer->CC[2].CTRL = TIMER_CC_CTRL_ICEVCTRL_RISING | TIMER_CC_CTRL_ICEDGE_RISING | TIMER_CC_CTRL_CMOA_CLEAR | TIMER_CC_CTRL_COIST | TIMER_CC_CTRL_MODE_PWM;
        timer->CC[2].CCVB = 0xFFFF;
    }
...
int PWM_Config(TIMER_TypeDef *timer, unsigned div, unsigned top)
{
    ...
    timer->CTRL = (timer->CTRL & ~(_TIMER_CTRL_PRESC_MASK | _TIMER_CTRL_MODE_MASK)) | (d << _TIMER_CTRL_PRESC_SHIFT) | (TIMER_CTRL_MODE_UP);
